...
function showHome(username, avatarUrl) {
  ...
  document.getElementById("homeUsername").textContent = `Welcome back, ${username}!`;
}
...
document.getElementById("registrationForm").addEventListener("submit", async (e) => {
  ...
  setTimeout(() => showHome(username, data.user.user_metadata?.avatar_url || null), 800);
});
...
supabase.auth.onAuthStateChange(async (event, session) => {
  if (session?.user) {
    ...
    const { data: userTableData } = await supabase
      ...
      .eq("id", user.id)
      .single();
    const username = userTableData?.username || user.email;
